<template>
    <div>
        ...
        <popup v-show="popupFlag" text="Оплата принята">
            <router-link :to="{name: 'add'}" slot="btn">Добавление</router-link>
        </popup>
    </div>
</template>
        ...
        mounted() {
            this.$store.dispatch('getCard')
                .then(data => {
                    ...
                    this.formData.date_month = ('0' + (date.getMonth() + 1)).slice(-2);
                    this.formData.date_year = (date.getFullYear()-(date.getFullYear()%Math.pow(10, 2)))/Math.pow(10, 2);
                });
        },
        methods: {
            editCard(e) {
